...
- **Daily loop** — the home screen leads with **Continue** (the next unmastered lesson, auto-starts) and a **Daily groove** (one chart a day in your current level band, +60 XP). Once you have cleared a chart, the daily carries a rotating twist worth +90 XP: **Tempo +10%**, **Clean** (no misses), **Fade** (notes vanish before the line, play from memory), **Tight** (three stars or nothing), or a plain Standard day. The twist's rule decides whether the Perform run clears the daily; the bonus is paid once per day. A scored Perform grows a streak, awards XP toward ranks (Rookie → Legend), and unlocks badges. Every 7 days of streak earns a **streak freeze** (max 2): miss one day and the next Perform spends a freeze to bridge the gap instead of resetting. The home screen shows the streak as *at risk* once yesterday's goal is the last one met, and as *frozen* when a freeze will save it. A **daily XP goal** (150 XP) fills a ring on the home screen and a bar on the results card. Practice steps still pay a little XP so they feel like play; only Perform counts for stars, streak, and the daily bonus. Each practice step is ticked off once you finish it in Practice or reach one star in Play; lesson cards show a dot per step, and a lesson reopens on the first step you haven't done yet. Three stars at 100% unlocks that lesson's **tempo ladder**: Perform can be replayed at 105 / 110 / 115 / 120 %, a rung is mastered with 3 stars at that tempo, faster runs pay more XP (120 % is 1.4×) plus a rung bonus, and the card shows the fastest tempo mastered.
...
- **Groove goals** — every Play run has three small goals sized to your current part: land 75% of its notes, hit 60% with Perfect or Great timing, and finish half its two-bar phrases without missing a charted note (targets round up). Phrase checkpoints light separately: **Landed** means every note hit; **Locked** means every note was Perfect or Great. A rough phrase leaves the earlier checkpoints intact, and the next phrase is a fresh chance. Empty backing-only phrases do not count. Results collect the goals and phrase checkpoints into a recap. Goals are feedback for this run; they do not award extra XP or change stars, daily clears, or lesson completion. Extras still affect the normal accuracy score but do not undo charted-note goals. Listen and wait-mode Practice have no scored goals.
- **Beat your best** — Perform at full tempo shows a target one percentage point above your best retained performance at the same lesson, tempo, note count, and daily rules. At 100%, the target is to match it. The target stays fixed for the run and is judged on the final accuracy, including extra hits. No comparable history means no invented target. Slowed steps and focus drills keep their groove goals without a full-performance target.
- **Jam studio** — open Jam from the studio to make your own four-bar beat on the 16-pad kit. Record gives you a four-beat count-in, captures one pass, then loops it. Overdub joins at the next loop boundary and adds another four-bar take. Mute individual recorded pad parts while the loop plays; your live pads stay audible. Stop cancels an unfinished take, keeping the previous loop. Undo restores the previous take or a cleared loop. Set 60–180 BPM while stopped; optional sixteenth-note snap applies only to new hits. Free timing and pad velocity are preserved when snap is off. The latest named sketch, tempo, snap setting, and muted parts save locally in `padlab-jam-v1`; the studio reports when saving is unavailable and retains the sketch during the current app visit. This records pad notes, not microphone audio. Jam never changes lesson progress, XP, streaks, daily clears, or quick sessions.
